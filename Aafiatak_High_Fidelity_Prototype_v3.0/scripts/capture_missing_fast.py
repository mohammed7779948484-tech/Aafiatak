...
for p in sorted((ROOT/'src/ts/screens').glob('PAT*.ts')):
 t=p.read_text(encoding='utf-8');sid=re.search(r"id:'(PAT-\d+)'",t).group(1);title=re.search(r"title:'([^']+)'",t).group(1);default=re.search(r"defaultState:'([^']+)'",t).group(1);sts=re.findall(r"'([^']+)'",re.search(r"states:\[([^\]]+)\]",t,re.S).group(1));manifest.append({'id':sid,'title':title,'default':default,'states':sts})
...
with sync_playwright() as pw:
 ...
 for rec in manifest:
  for st in rec['states']:
   fn=f"{rec['id']}-{st}-phone.png";pth=PREVIEW/fn
   errs=[]
   if not pth.exists():
    p.set_content(html(rec['id'],st),wait_until='load');m=p.locator('body').evaluate('(e)=>({w:e.scrollWidth,h:e.scrollHeight,cw:e.clientWidth})');p.screenshot(path=str(pth),full_page=True,scale='css')
   else:
    p.set_content(html(rec['id'],st),wait_until='load');m=p.locator('body').evaluate('(e)=>({w:e.scrollWidth,h:e.scrollHeight,cw:e.clientWidth})')
   reports.append({'screen':rec['id'],'state':st,'file':fn,'h':m['h'],'overflow':m['w']>m['cw'],'errors':errs})
 p.close();b.close()
...
for rec in manifest:
 ...
 cols=3;rows=(len(tiles)+2)//3;sheet=Image.new('RGB',(cols*250+(cols+1)*12,rows*625+56),'#EEE8E1');d=ImageDraw.Draw(sheet);d.text((18,20),f"{rec['id']} states",fill='#4A315D')
